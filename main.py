...
def quiz():
    ...
    if len(questions) < 11:  # Eğer yeterli yeni soru yoksa, tekrar sorular gösterilebilir
        questions = Question.query.order_by(db.func.random()).limit(41).all()
...
def add_questions():
    if Question.query.count() == 0:  # Eğer veritabanında hiç soru yoksa ekle
        ...
        Question(text="Hansılardan biri yanğının baş verməsi üçün lazım deyil?", option_a="Yanacaq", option_b="Oksigen", option_c="Qapalı mühit", option_d="Kifayət qədər enerjili qığılcım", correct_option="C"),
        Question(text="Hansılardan hansı qaz atmosferini alışdıra bilər?", option_a="Elektrik qığılcımı", option_b="İsti səth", option_c="Batareya", option_d="İntrinsik təhlükəsiz dövrə", option_e="Statik elektrik", option_f="Şimşək", option_g="Nəqliyyat vasitəsi", option_h="Sürtünmə", correct_option=["A", "B", "E", "F", "H"]),
        Question(text="Qazın öz-özünə alışdığı temperatur necə adlanır?", option_a="Ətraf mühit temperaturu", option_b="Alışma temperaturu", option_c="Yanma nöqtəsi", option_d="Termal temperatur", correct_option="B"),
        Question(text="Aşağıdakılardan hansı təsnifat daha çox istifadə olunur?", option_a="Zona 0, ildə 2000 saatdan çox partlayıcı atmosfer", option_b="Zona 0, ildə 1000 saatdan çox partlayıcı atmosfer", option_c="Zona 2, ildə 10 saatdan az partlayıcı atmosfer", option_d="Zona 1, ildə 10-1000 saat arasında partlayıcı atmosfer", correct_option=["A", "C", "D"]),
        Question(text="Hansılardan biri I qrup qaz nümunəsidir?", option_a="Metan", option_b="Propan", option_c="Etilen", option_d="Hidrogen və asetilen", correct_option="A"),
        Question(text="ATEX 95 Direktivi hansı məqsədlərə xidmət edir?", option_a="Texniki ticarət maneələrinin aradan qaldırılması", option_b="Risk qiymətləndirmələrinin daxil edilməsi", option_c="Avropada 230V AC standartlaşdırılması", option_d="İşçilərin sağlamlığı və təhlükəsizliyi", correct_option=["A", "B", "D"]),
        Question(text="ATEX kateqoriyaları ilə əlaqədar, avadanlıqlar üçün hansı qısaltma istifadə olunur?", option_a="IEC", option_b="APL", option_c="EPL", option_d="API", correct_option="C"),
        Question(text="EPL-in açılışı nədir?", option_a="Elektrik Mühafizə Səviyyəsi", option_b="Avadanlıq Mühafizə Səviyyəsi", option_c="Elektrik Mühafizə Yükü", option_d="Avadanlıq Təzyiq Səviyyəsi", correct_option="B"),
        Question(text="Ex 'n' avadanlıqları harada istifadə edilə bilər?", option_a="Zona 0", option_b="Zona 0 və Zona 1", option_c="Zona 0, 1 və 2", option_d="Zona 2", correct_option="D"),
        Question(text="'Gc' avadanlıq mühafizə səviyyəsi (EPL) və ya 3-cü kateqoriya avadanlıqlar harada istifadə edilə bilər?", option_a="Zona 0", option_b="Zona 0 və 1", option_c="Zona 0, 1 və 2", option_d="Zona 2", correct_option="D"),
        Question(text="Qığılcım yaradan komponentlər (məsələn, kontaktlar və açarlar) Ex 'd' örtüklərinin içərisində yerləşdirilə bilərmi?", option_a="Bəli", option_b="Xeyr", correct_option="A"),
        Question(text="Ex 'd' mühafizə konsepsiyasının ümumi tələbləri ilə bağlı hansı standart istifadə olunur?", option_a="IEC 60079-1", option_b="IEC 60079-3", option_c="IEC 60079-14", option_d="IEC 60079-17", correct_option="A"),
        Question(text="Alovkeçirməz avadanlıqlar niyə belə adlanır?", option_a="Daxili partlayışa icazə verilmir", option_b="Daxili partlayışa icazə verilir", option_c="Xarici partlayışa icazə verilir", option_d="Qazın örtük içinə girməsinə müqavimət göstərir", correct_option="B"),
        Question(text="Ex 'd' avadanlıqları harada istifadə edilə bilər?", option_a="Zona 0", option_b="Zona 1 və 2", option_c="Yalnız Zona 2", option_d="Zona 0 və 1", correct_option="B"),
        Question(text="IEC 60079-1 standartı nə ilə bağlıdır?", option_a="Ex 'd' alovkeçirməz mühafizə konsepsiyası", option_b="Ex 'e' artırılmış təhlükəsizlik konsepsiyası", option_c="Ex 'i' intrinsik təhlükəsizlik konsepsiyası", option_d="Ex 'n' normal mühafizə konsepsiyası", correct_option="A"),
        Question(text="Alovkeçirməz (flameproof) örtükdə yerləşən açarın həcmi 2 litrdən azdır və IIC mühitində yerləşir. Örtük hansı növ qovşaqlara ehtiyac duyur?", option_a="Sənaye tipli qovşaqlar", option_b="Ex 'e' qovşaqlar", option_c="Ex 'd' baryer qovşaqlar", option_d="Dolğu qovşaqları ilə təchiz olunmuş dolayı giriş Ex 'e' kamerası", correct_option="C"),
        Question(text="Artırılmış təhlükəsiz Ex 'e' aparatları daxili partlayışları saxlayacaq şəkildə hazırlanmışdır?", option_a="Bəli", option_b="Xeyr", option_c="Bəli, əgər baryer qovşaqlar istifadə olunarsa", option_d="Bəli, əgər IP dərəcəsi ən azı IP54 olarsa", correct_option="B"),
        Question(text="Ümumiyyətlə, Ex 'e' avadanlıqları harada quraşdırıla bilər?", option_a="Zona 0", option_b="Zona 1 və 2", option_c="Zona 0, 1 və 2", option_d="Yalnız Zona 2", correct_option="B"),
        Question(text="Ex 'e' avadanlıqları hansı EPL (Equipment Protection Level) tələblərinə cavab verməlidir?", option_a="EPL 'Ga' və 'Gb' avadanlıqları", option_b="EPL 'Ga', 'Gb' və 'Gc' avadanlıqları", option_c="EPL 'Ga' və ya 1-ci kateqoriya avadanlıqları", option_d="EPL 'Gb' və 'Gc' avadanlıqları", correct_option="D"),
        Question(text="Ex 'e' avadanlıqlarının IP reytinqi neçə olmalıdır?", option_a="IP 54", option_b="Ən azı IP 54, lakin mühit şərtləri daha yüksək dəyər tələb edə bilər", option_c="IP 44", option_d="Ən azı IP 44, lakin mühit şərtləri daha yüksək dəyər tələb edə bilər", correct_option="B"),
        Question(text="Ex 'e' avadanlıqlarda kontrqaykalar (locknut) nə vaxt tələb olunur?", option_a="Həmişə tələb olunur", option_b="Heç vaxt tələb olunmur", option_c="Yalnız boşluq deşiklərində tələb olunur", option_d="Yalnız 10 mm qalınlığında yivli girişlərdə tələb olunur", correct_option="C"),
        Question(text="Ümumiyyətlə, Ex 'n' avadanlıqları harada quraşdırıla bilər?", option_a="Zona 0", option_b="Zona 1", option_c="Zona 2", option_d="Təmiz zonalar", correct_option="C"),
        Question(text="Ex 'n' avadanlıqları hansı qaz qrupları üçün nəzərdə tutulmuş ola bilər?", option_a="II qrup", option_b="IIA və IIB qrupları", option_c="Yalnız IIA qrupu", option_d="Yalnız IIB qrupu", correct_option="B"),
        ...
        Question(text="8 mm qalınlığında, yivli girişə malik Ex 'n' metal örtüyə kabel qoşulduqda hansı tələb olunur?", option_a="Kontrqayka (locknut)", option_b="Ulduz yuyucu (star washer)", option_c="IP yuyucu (IP washer)", option_d="Heç biri", correct_option="A"),
        Question(text="Ex 'n' məhdud nəfəsalma (restricted breathing) mühafizəsi üçün həmişə hansı element təmin edilməlidir?", option_a="IP yuyucu (IP washer)", option_b="Kontrqayka (locknut)", option_c="Yerə qoşulma etiketi (earth tag)", option_d="Yivli giriş (threaded entry)", correct_option="D"),
        Question(text="Ex 'ia' avadanlıqları harada quraşdırıla bilər?", option_a="Zona 0", option_b="Zona 1", option_c="Zona 2", option_d="Təmiz zonalar", correct_option="A"),
        Question(text="Qalvanik izolatorlar nəyi tələb edir?", option_a="İntrinsik təhlükəsiz yerə qoşulma", option_b="Ən azı iki Zener diodu", option_c="Elektrik ayrılmasını təmin edən qurğu (məsələn, transformator, relye və ya opto-kuplör)", option_d="Ən azı üç Zener diodu", correct_option="C"),
        Question(text="Aşağıdakılardan hansına intrinsik təhlükəsiz xüsusi yerə qoşulma lazım deyil?", option_a="İki Zener diodlu Zener baryeri", option_b="Qalvanik izolyasiyalı baryer", option_c="Üç Zener diodlu Zener baryeri", option_d="Sadə aparatları təmin edən Zener baryeri", correct_option="B"),
        Question(text="İntrinsik təhlükəsiz dövrələrin terminalları ilə qeyri-intrinsik təhlükəsiz dövrələrin terminalları arasında minimum məsafə nə qədər olmalıdır?", option_a="100 mm", option_b="50 mm", option_c="6 mm", option_d="3 mm", correct_option="C"),
        Question(text="Yerə qədər olan minimum məsafə (intrinsik təhlükəsiz dövrələr üçün) nə qədərdir?", option_a="100 mm", option_b="50 mm", option_c="6 mm", option_d="3 mm", correct_option="D")
        ]
